...
[ADR-0014](./0014-authoring-and-datastore.md) は、採用した選択肢の理由として「公開できない値を、出力する場所の1箇所で選別できる」を挙げた。**その 1 箇所をどこに置くかは決めていない。** 2026-09-08 時点で、選別を行うコードはリポジトリに存在しない。
...
* synsk が 2026-09-08 に示した判断の優先順位。読めること、後から覆せること、漏れないことの順とする
...
- 公開経路はテーブルを直接読まない
- ビューの名前は、テーブルの名前を部分文字列として含まない形にする

可視性の段階ごとの粒度は本 ADR では扱わない。
...
2. 公開経路がテーブル名を直接書いていないことを確かめる。ビューの名前だけが現れることを検査する。Decision Outcome が定めた命名がこの検査を成立させる。`career` と `career_public` のように一方が他方を部分文字列として含む組では、素朴な文字列の一致が必ず誤って検出する
