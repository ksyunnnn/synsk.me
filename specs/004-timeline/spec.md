...
- Q: timeline をどの経路に置くか → A: トップページ（`/`）に置く。UI としてはページではなくコンポーネントとして扱い、timeline 自身は固有の経路を持たない
...
- **FR-013**: timeline はトップページ（`/`）に置かれる。timeline 自身は固有の経路を持たず、複数の経路に置ける表示単位である
...
- **SC-006**: timeline を置いたトップページが `docs/REQUIREMENTS.md` の NFR-03〜NFR-07（LCP・INP・CLS・TTFB・FCP）を満たす
